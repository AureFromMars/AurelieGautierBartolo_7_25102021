...
exports.likingMessage = (req, res, next) => {
  ...
  .then (message => {
    const likingDatas = {
      userLiking: message.userLiking,
      // usersDisliked: message.usersDisliked,
      liking: message.liking
      // dislikes: message.dislikes
    };
    switch (req.body.liking) {
      case -1 :// Dislike case
        if (likingDatas.userLiking.includes(req.body.userId)) {
          const indexUsersLiking = likingDatas.userLiking.indexOf(req.body.userId);
          likingDatas.userLiking.splice(indexUsersLiking, 1)
        };
      ...
      case 1 :// Like case
        ...
        // };
        if (!likingDatas.userLiking.includes(req.body.userId)) {// To protect multiple dislike
          likingDatas.userLiking.push(req.body.userId);
        };
        console.log("Like");
        break;
    ...
    likingDatas.liking = likingDatas.userLiking.length;
